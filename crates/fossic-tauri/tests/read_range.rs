...
fn read_range_bounded_no_cursor_returns_complete() {
    ...
    for i in 0..5u32 {
        store.append(Append { stream_id: "test/b".into(), event_type: "E".into(), payload: serde_json::json!({"i": i}), ..Default::default() }).unwrap();
    }
    let app = make_app(store);
    let state = app.state::<Store>();
    let outcome = fossic_tauri::commands::fossic_read_range_bounded(
        state, "test/b".into(), None, None, None, None, None, None,
...
fn read_range_bounded_truncates_at_max_results() {
    ...
    for i in 0..10u32 {
        store.append(Append { stream_id: "test/b2".into(), event_type: "E".into(), payload: serde_json::json!({"i": i}), ..Default::default() }).unwrap();
    }
    let app = make_app(store);
    let state = app.state::<Store>();
    let outcome = fossic_tauri::commands::fossic_read_range_bounded(
        state, "test/b2".into(), None, None, None, None, Some(3), None,
    ).map_err(|e| e.message).unwrap();
...
fn read_range_from_cursor_resumes_correctly() {
    ...
    for i in 0..6u32 {
        store.append(Append { stream_id: "test/b3".into(), event_type: "E".into(), payload: serde_json::json!({"i": i}), ..Default::default() }).unwrap();
    }
    let app = make_app(store);
    let state = app.state::<Store>();

    let page1 = fossic_tauri::commands::fossic_read_range_bounded(
        state.clone(), "test/b3".into(), None, None, None, None, Some(3), None,
    ).map_err(|e| e.message).unwrap();
    ...
    let page2 = fossic_tauri::commands::fossic_read_range_from_cursor(
        state, "test/b3".into(), None, None, None, None, Some(3), None, cursor,
    ).map_err(|e| e.message).unwrap();
...
fn read_range_bounded_full_pagination() {
    ...
    for i in 0..7u32 {
        store.append(Append { stream_id: "test/b4".into(), event_type: "E".into(), payload: serde_json::json!({"i": i}), ..Default::default() }).unwrap();
    }
    let app = make_app(store);
    let state = app.state::<Store>();
    ...
    loop {
        let outcome = if let Some(c) = cursor {
            fossic_tauri::commands::fossic_read_range_from_cursor(
                state.clone(), "test/b4".into(), None, None, None, None, Some(3), None, c,
            ).map_err(|e| e.message).unwrap()
        } else {
            fossic_tauri::commands::fossic_read_range_bounded(
                state.clone(), "test/b4".into(), None, None, None, None, Some(3), None,
            ).map_err(|e| e.message).unwrap()
        };
...
fn read_by_correlation_bounded_paginates() {
    ...
    let root = store.append(Append { stream_id: "test/c".into(), event_type: "Root".into(), payload: serde_json::json!({}), ..Default::default() }).unwrap();
    for i in 0..5u32 {
        store.append(Append { stream_id: "test/c".into(), event_type: "Child".into(), payload: serde_json::json!({"i": i}), correlation_id: Some(root), ..Default::default() }).unwrap();
    }
    let app = make_app(store);
    let state = app.state::<Store>();
    let outcome = fossic_tauri::commands::fossic_read_by_correlation_bounded(
        state, root.to_hex(), Some(3), None,
    ).map_err(|e| e.message).unwrap();
    assert_eq!(outcome.kind, "truncated");
    assert_eq!(outcome.results.len(), 3);
    assert!(outcome.next_cursor.is_some());
...
fn read_by_correlation_from_cursor_resumes() {
    ...
    let root = store.append(Append { stream_id: "test/c2".into(), event_type: "Root".into(), payload: serde_json::json!({}), ..Default::default() }).unwrap();
    for i in 0..6u32 {
        store.append(Append { stream_id: "test/c2".into(), event_type: "Child".into(), payload: serde_json::json!({"i": i}), correlation_id: Some(root), ..Default::default() }).unwrap();
    }
    let app = make_app(store);
    let state = app.state::<Store>();

    let page1 = fossic_tauri::commands::fossic_read_by_correlation_bounded(
        state.clone(), root.to_hex(), Some(3), None,
    ).map_err(|e| e.message).unwrap();
    assert_eq!(page1.kind, "truncated");
    let cursor = page1.next_cursor.clone().unwrap();

    let page2 = fossic_tauri::commands::fossic_read_by_correlation_from_cursor(
        state, root.to_hex(), Some(3), None, cursor,
    ).map_err(|e| e.message).unwrap();
    assert_eq!(page2.results.len(), 3);
    // ids from both pages must be disjoint
    let ids1: std::collections::HashSet<&str> = page1.results.iter().map(|e| e.id.as_str()).collect();
    let ids2: std::collections::HashSet<&str> = page2.results.iter().map(|e| e.id.as_str()).collect();
...
fn walk_causation_bounded_forward() {
    ...
    let root = store.append(Append { stream_id: "test/chain".into(), event_type: "N".into(), payload: serde_json::json!({}), ..Default::default() }).unwrap();
    let mut prev = root;
    for i in 0..4u32 {
        let id = store.append(Append { stream_id: "test/chain".into(), event_type: "N".into(), payload: serde_json::json!({"i": i}), causation_id: Some(prev), ..Default::default() }).unwrap();
        prev = id;
    }
    let app = make_app(store);
    let state = app.state::<Store>();
    let outcome = fossic_tauri::commands::fossic_walk_causation_bounded(
        state, root.to_hex(), "forward".into(), None, None, None, None,
    ).map_err(|e| e.message).unwrap();
    assert_eq!(outcome.kind, "complete");
...
fn walk_causation_bounded_truncates_at_max_results() {
    ...
    let root = store.append(Append { stream_id: "test/chain2".into(), event_type: "N".into(), payload: serde_json::json!({}), ..Default::default() }).unwrap();
    let mut prev = root;
    for i in 0..6u32 {
        let id = store.append(Append { stream_id: "test/chain2".into(), event_type: "N".into(), payload: serde_json::json!({"i": i}), causation_id: Some(prev), ..Default::default() }).unwrap();
        prev = id;
    }
    let app = make_app(store);
    let state = app.state::<Store>();
    let outcome = fossic_tauri::commands::fossic_walk_causation_bounded(
        state, root.to_hex(), "forward".into(), None, None, Some(3), None,
    ).map_err(|e| e.message).unwrap();
    assert_eq!(outcome.kind, "truncated");
    assert_eq!(outcome.results.len(), 3);
    assert!(outcome.next_cursor.is_some());
...
fn aggregate_bounded_cross_stream() {
    ...
    for i in 0..3u32 {
        store.append(Append { stream_id: "agg/a".into(), event_type: "E".into(), payload: serde_json::json!({"i": i, "s": "a"}), ..Default::default() }).unwrap();
        store.append(Append { stream_id: "agg/b".into(), event_type: "E".into(), payload: serde_json::json!({"i": i, "s": "b"}), ..Default::default() }).unwrap();
    }
    let app = make_app(store);
    let state = app.state::<Store>();
    let outcome = fossic_tauri::commands::fossic_aggregate_bounded(
        state, "agg/*".into(), None, None, None, None, None, None, None,
    ).map_err(|e| e.message).unwrap();
    assert_eq!(outcome.kind, "complete");
...
fn aggregate_bounded_truncates_at_max_events() {
    ...
    for i in 0..10u32 {
        store.append(Append { stream_id: "agg2/a".into(), event_type: "E".into(), payload: serde_json::json!({"i": i}), ..Default::default() }).unwrap();
    }
    let app = make_app(store);
    let state = app.state::<Store>();
    let outcome = fossic_tauri::commands::fossic_aggregate_bounded(
        state, "agg2/*".into(), None, None, None, None, None, Some(4), None,
    ).map_err(|e| e.message).unwrap();
    assert_eq!(outcome.kind, "truncated");
    assert_eq!(outcome.results.len(), 4);
    assert_eq!(outcome.reason.unwrap(), "result_count");
